pipeline{
    ...
    agent { label "master" }
    parameters {
        string(name: 'AMIID', defaultValue: 'use_latest', description: 'AMI to use. Or use_latest to pull from paramater store')
    }

    environment {
        // Tags
        AWS_ACCOUNT_ID = "143049391535"
        AWS_ACCOUNT_NAME ="entplat"
        APPNAME_TAG = "example"
        APPID_TAG = "1005238"
        ENV_TAG = "dev"
        OWNER_TAG = "example@baxter.com"
        COSTCENTER_TAG = "1001700224"
        // Vars for the packer builds
        SECURITY_GROUPS = 'sg-0795946d'
        SUBNET = 'subnet-8b19ffe0'
        REGION = 'eu-central-1'
        AMI_NAME = "${APPNAME_TAG}-win-2016"
        AMI_BUILD_FOLDER = "amibuilds/win_2016"
        ...
        EBS_ENCRYPTION_KEY_EU_CENTRAL_1 = 'arn:aws:kms:eu-central-1:143049391535:key/6e9d855c-799b-43ed-b260-af2329546608'
        // Paramater store values and prefix
        SAVE_AMI_SSM_PARAM_PREFIX="/${APPNAME_TAG}/ami/windows/2016/"
        EIS_AMI_SSM_PARAM="/eis/ami/windows/2016/latest"
        //Runway var
        ...
        stage("Create AMI using packer") {
            steps {
                sh '''
                    if [ ${AMIID} = "use_latest" ]
                    then
                        AMIID=$(aws ssm get-parameter --name "$EIS_AMI_SSM_PARAM" --query "Parameter.Value" --output text --region $REGION)
                    fi
        ...
        stage ('Copy and encrypt AMI in all regions') {
            steps {
                sh '''
                    ...
                    UNENCRYPTED_SHARING_IMAGE=$(cat ${AMI_BUILD_FOLDER}/ami.txt)
